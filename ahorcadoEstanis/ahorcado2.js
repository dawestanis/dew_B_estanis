...
btnComenzar.addEventListener('click', function(){
        ...
        function loadFile(url){
            ...
            rawFile.onreadystatechange = function(){
                console.log("Entra+ "*rawFile.status) 
                if(rawFile.readyState === 4 && rawFile.status == "200"){
                     pelicula = JSON.parse(rawFile.responseText);
                    let size = pelicula.length
                    //RANDOM
                    //pelicula = pelicula[random]
                }
            }
...
function comenzarPartida() { //funcion que nos permite comenzar la partida
    ...
    partida = new Partida(pelicula.titulo, j1, j2);
    partida.ponerBarra();
    actualizarDatos();
